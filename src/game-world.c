...
void process_player(void) {
    ...
    PROFILE("player_resting_complete_special",player_resting_complete_special(player);)

    PROFILE("EVENT_CHECK_INTERRUPT", event_signal(EVENT_CHECK_INTERRUPT);)

    /* Repeat until energy is reduced */
    do {
        /* Refresh */
        PROFILE("notice_stuff(player); 1", notice_stuff(player);)
        PROFILE("handle_stuff(player);",handle_stuff(player);)
        PROFILE("event_signal(EVENT_REFRESH); 1",event_signal(EVENT_REFRESH);)
        ...
        if (cmd_get_nrepeats() > 0) {
            PROFILE("event_signal(EVENT_COMMAND_REPEAT);",event_signal(EVENT_COMMAND_REPEAT);)
        } else {
            ...
            PROFILE("event_signal(EVENT_REFRESH); 2",event_signal(EVENT_REFRESH);)
        }

        /* Get a command from the queue if there is one */
        PROFILE("if (!cmdq_pop(CTX_GAME))", if (!cmdq_pop(CTX_GAME)) {
            pop_profile();
            break;
            }
        )
    ...
    PROFILE("notice_stuff(player); 2",notice_stuff(player);)
}
 ...
 */
/* Helpful regex */
// PROFILE(/* comment */, <code>) => stringized version
// Find   : "/\* ([^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?[^"]?)([^"]|\n)+ */"
// Replace: "$1", // $0

// Find: r3turn
// Replace: PROFILE_RETURN

void run_game_loop(void) {
    PROFILE("Tidy up after the ",  // "/* Tidy up after the player's command */",
            process_player_cleanup();)

    PROFILE(
        "Keep processing th",  // "/* Keep processing the player until they use some energy or\
	 * another command is needed */",
        while (player->upkeep->playing) {
            PROFILE("process_player();",process_player();)
            if (player->upkeep->energy_use)
                break;
            else
                PROFILE_RETURN(1);  ///////////////////////////////////////////////////////////////;
    ...
    PROFILE(
        "The player may sti",  // "/* The player may still have enough energy to move, so we run another\
	 * player turn before processing the rest of the world */",
        while (player->energy >= z_info->move_energy) {
            ...
            while (player->upkeep->playing) {
                PROFILE("process_player(); 2", process_player();)
                if (player->upkeep->energy_use)
                    break;
                else
                    PROFILE_RETURN(1);  ///////////////////////////////////////////////////////////////
    ...
    PROFILE(
        "Now that the playe",  // "/* Now that the player's turn is fully complete, we run the main loop \
	 * until player input is needed again */",
        while (true) {
            PROFILE("notice_stuff(player); -1",notice_stuff(player);)
            PROFILE("handle_stuff(player); -1", handle_stuff(player);)
            PROFILE("event_signal(EVENT_REFRESH); -1", event_signal(EVENT_REFRESH);)
		 ...
		 * generate a new level */
            if (player->is_dead || !player->upkeep->playing)
                ...
                PROFILE("Process the rest o",  // "/* Process the rest of the monsters */",
                        process_monsters(0);)

                PROFILE("Mark all monsters ",  // "/* Mark all monsters as ready to act when they have the energy */",
                        reset_monsters();)

            PROFILE("notice_stuff(player); -2",notice_stuff(player);)
            PROFILE("handle_stuff(player); -2", handle_stuff(player);)
            PROFILE("event_signal(EVENT_REFRESH); -2", event_signal(EVENT_REFRESH);)
                ...
                PROFILE(
                    "Process the world ",  // "/* Process the world every ten turns */",
                    if (!(turn % 10) && !player->upkeep->generate_level) {
                        PROFILE("process_world(cave);",process_world(cave);)

                        /* Refresh */
                        PROFILE("notice_stuff(player); -3",notice_stuff(player);)
                        PROFILE("handle_stuff(player); -3", handle_stuff(player);)
                        PROFILE("event_signal(EVENT_REFRESH); -3", event_signal(EVENT_REFRESH);)
            ...
            PROFILE(
                "If the player has ",  // "/* If the player has enough energy to move they now do so, after\
		 * any monsters with more energy take their turns */",
                while (player->energy >= z_info->move_energy) {
                    PROFILE("Do any necessary a",  // "/* Do any necessary animations */",
                            event_signal(EVENT_ANIMATE);)

                    PROFILE("Process monster wi",  // "/* Process monster with even more energy first */",
                            process_monsters(player->energy + 1);)
                    if (player->is_dead || !player->upkeep->playing ||
                        player->upkeep->generate_level)
                        break;
                    ...
                    PROFILE(
                        "Process the player",  // "/* Process the player until they use some energy */",
                        while (player->upkeep->playing) {
                            process_player();
                            if (player->upkeep->energy_use)
                                break;
